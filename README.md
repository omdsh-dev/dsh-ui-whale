...
> - DSH **0.1.1-rc.2**（npm 最新）：装**旧版** `'@dsh-external/dsh-ui-whale@github:dsh-external/dsh-ui-whale#v0.3.4'`
...
> 1. 执行 `dsh plugin --profile web add '@dsh-external/dsh-ui-whale@github:dsh-external/dsh-ui-whale#v0.3.8'`（首次可能被 pnpm 11 拦截 node-pty 构建脚本而失败）
...
| `v0.3.8`（默认） | `dsh-v0.1.2-alpha.1`（GitHub tag，源码构建安装） | 当前版本：迁移到 0.1.2-alpha.1 客户端 API（views/legacy 投影 + ctx.slots.inject），带兼容自诊断横幅 |
...
> git 依赖方式固定 tag（公开镜像，推荐）：`pnpm add '@dsh-external/dsh-ui-whale@github:dsh-external/dsh-ui-whale#v0.3.8'`（或 `github:dsh-external/dsh-ui-whale`；历史版本：0810/0811 用户用 `#v0.3.3`，0806~0809 用户用 `#v0.3.2`，0805 用户用 `#v0.1.0`）。
