window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		//#region src/client/animation.ts
		...
		*/
		function advance(state, mood, heartRequested = false) {
			...
			const tail = advanceLimb(state.wagStep, state.wagHold, state.thumpCountdown, WAG_SEQUENCE, WAG_HOLD[mood], IDLE_THUMP_GAP, continuous);
			const fin = advanceLimb(state.finStep, state.finHold, state.flutterCountdown, FIN_SEQUENCE, FIN_HOLD[mood], IDLE_FLUTTER_GAP, continuous);
		...
		const FRAMES = {
			...
			"TAIL_2": [
				...
				"0000000000000000000000000121000000001100",
				"0000000000000000000000000122100000012100",
				"0000000000000000000000000122210000122100",
				"0000000000000000000000000012221011221000",
				"0000000111111111000000000001222122221000",
				"0000001222222222110000000001222222210000",
				"0000012222222222221100000000122222100000",
		...
		*/
		...
		const BODY = framePixels(FRAMES.STANDARD).filter((p) => !EYE_CELLS.has(`${p.x},${p.y}`) && !TAIL.has(`${p.x},${p.y}`) && !FIN.has(`${p.x},${p.y}`) && !SPOUT.has(`${p.x},${p.y}`) && !HEART.has(`${p.x},${p.y}`));
		...
		BODY.length, TAIL_FRAMES.map((f) => f.length), FIN_FRAMES.map((f) => f.length), SPOUT_FRAMES.map((f) => f.length), HEART_FRAMES.map((f) => f.length), EYE_OPEN.length, EYE_CLOSED.length, TAIL_FRAMES.length, FIN_FRAMES.length, SPOUT_FRAMES.length, HEART_FRAMES.length;
		//#endregion
		//#region \0dsh-css:E:\deepseek-harness\dsh-ui-whale\src\client\WhalePet.module.css.mjs
		const css = "._8cizTW_pet{--whale-c1:#203864;--whale-c2:#06f;--whale-c3:#b4c7e7;--whale-c4:#f2f2f2;--whale-c5:#c39;--whale-scale:2;width:calc(40px * var(--whale-scale));height:calc(25px * var(--whale-scale));cursor:pointer;flex:none;position:relative}._8cizTW_pixels{width:1px;height:1px;box-shadow:var(--whale-shadows);transform:scale(var(--whale-scale));transform-origin:0 0;position:absolute;top:0;left:0}@media (prefers-reduced-motion:reduce){._8cizTW_pet{--whale-scale:2}}";
		...
		*/
		function WhalePet({ useSession, t }) {
