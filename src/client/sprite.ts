...
/** One sprite pixel: column, row, and palette key (1 dark, 2 body, 3 light, 4 white). */
...
const BODY: readonly Pixel[] = framePixels(FRAMES.STANDARD)
  .filter(p => !EYE_CELLS.has(`${p.x},${p.y}`) && !TAIL.has(`${p.x},${p.y}`) && !FIN.has(`${p.x},${p.y}`) && !SPOUT.has(`${p.x},${p.y}`))
...
/** The pet's visible frame: which eye/tail/fin/spout sub-frames are showing right now. */
...
export function framePixelsFor(frame: WhaleFrame): readonly Pixel[] {
  ...
  return [...BODY, ...eyes, ...tail, ...fin, ...spout]
}
...
/** The resting frame (eyes open, tail/fins/spout in their STANDARD poses). */
export const RESTING_FRAME: WhaleFrame = { tail: 0, fin: 0, spout: 0, blink: false }
